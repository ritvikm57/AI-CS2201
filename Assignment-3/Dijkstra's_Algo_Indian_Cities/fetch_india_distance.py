...
N = len(CITIES)
K = 6           # nearest neighbors to keep per city
DELAY = 0.25    # seconds between requests
API_URL = "https://maps.googleapis.com/maps/api/distancematrix/json"
BATCH = 25      # API max destinations per request
...
def fetch_distances_from(origin: str, destinations: list[str], api_key: str) -> list[int | None]:
    ...
    Returns a list of km values (None if unreachable).
    """
    params = {
        "origins":      origin,
...
def fetch_all_from(origin: str, destinations: list[str], api_key: str) -> list[int | None]:
    """
    Fetch distances from one origin to ALL destinations,
    batching into groups of BATCH to stay within API limits.
    """
    all_results = []
...
def main():
    ...
    print(f"    Total API requests: {N}  |  Total elements: ~{N * (N-1):,}  |  Est. cost: ~$0.03\n")
    ...
    # ── Build sparse edge list with UNION rule ────────────────────────────────
    #
    # Keep edge (i, j) if i is in j's top-K  OR  j is in i's top-K
    # Normalize as (min(i,j), max(i,j)) to avoid duplicates.
    #
    ...
    if kms:
        ...
        print(f"\n📊 Summary:")
        print(f"   Cities     : {N}")
        print(f"   Edges      : {len(edges):,}  (K={args.k}, union rule)")
        ...
        # Connectivity check — warn if any city has no edges
        city_mentions = {}
        for e in edges:
        ...
        if isolated:
            print(f"\n⚠️  Isolated cities (no edges — consider increasing --k):")
            for c in isolated:
                print(f"     {c}")
        else:
